...
from encrypticoin_ssi_tests.tracking.service_client import TrackingServiceClientMock


@pytest.fixture
def server_p():
    # NOTE: Production server must be served over HTTPS, but here it will bind to `http://127.0.0.1:8000`.
    server_p = subprocess.Popen([sys.executable, "-m", "encrypticoin_ssi_tests.tracking.service_server"])
    try:
        yield server_p
    finally:
...
@pytest.mark.asyncio
async def test_tracking_workflow(server_p):
    ...
    client0 = TrackingServiceClientMock()  # service-client and session of user
    ...
    assert len(await client0.debug_wallet_changes()) > 0
